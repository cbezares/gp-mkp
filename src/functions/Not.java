...
public class Not extends GPNode {
	...
	public void eval(
		...
		//state.output.println("Function.eval: Not", MKProblem.LOG_FILE);
		
		MKPData mkpd = (MKPData) input;
		MKProblem mkpp = (MKProblem) problem;
		
		children[0].eval(state, thread, mkpd, stack, individual, mkpp);
		//state.output.println("Resultado del \"NOT(before)\":\t" + mkpd.getResult(), MKProblem.LOG_FILE);
		
		mkpd.setResult(!mkpd.getResult());		
		//state.output.println("Resultado del \"NOT(after)\":\t" + mkpd.getResult(), MKProblem.LOG_FILE);
	}
}