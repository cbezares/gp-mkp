...
public class AddMaxNormalized extends GPNode {

	private static final long serialVersionUID = -2534468795456857335L;
	
	public String toString() { return "Add_Max_Normalized"; }
	...
	public void eval(final EvolutionState state, final int thread,
		...
		//state.output.println("Terminal.eval: addMaxNormalized", MKProblem.LOG_FILE);
		
		MKPData mkpd = (MKPData) input;
		
		//state.output.println("KS Antes:\t" + mkpd.getInstance().getKnapsack().toString(false), MKProblem.LOG_FILE);
		
		mkpd.setResult(MKP.addMaxNormalized(mkpd.getInstance().getKnapsack()));
